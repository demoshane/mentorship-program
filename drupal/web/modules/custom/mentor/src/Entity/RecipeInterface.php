<?php

namespace Drupal\mentor\Entity;

use Drupal\Core\Entity\RevisionLogInterface;
use Drupal\Core\Entity\RevisionableInterface;
use Drupal\Core\Entity\EntityChangedInterface;
use Drupal\user\EntityOwnerInterface;

/**
 * Provides an interface for defining Recipe entities.
 *
 * @ingroup mentor
 */
interface RecipeInterface extends RevisionableInterface, RevisionLogInterface, EntityChangedInterface, EntityOwnerInterface {

  // Add get/set methods for your configuration properties here.

  /**
   * Gets the Recipe name.
   *
   * @return string
   *   Name of the Recipe.
   */
  public function getName();

  /**
   * Sets the Recipe name.
   *
   * @param string $name
   *   The Recipe name.
   *
   * @return \Drupal\mentor\Entity\RecipeInterface
   *   The called Recipe entity.
   */
  public function setName($name);

  /**
   * Gets the Recipe creation timestamp.
   *
   * @return int
   *   Creation timestamp of the Recipe.
   */
  public function getCreatedTime();

  /**
   * Sets the Recipe creation timestamp.
   *
   * @param int $timestamp
   *   The Recipe creation timestamp.
   *
   * @return \Drupal\mentor\Entity\RecipeInterface
   *   The called Recipe entity.
   */
  public function setCreatedTime($timestamp);

  /**
   * Returns the Recipe published status indicator.
   *
   * Unpublished Recipe are only visible to restricted users.
   *
   * @return bool
   *   TRUE if the Recipe is published.
   */
  public function isPublished();

  /**
   * Sets the published status of a Recipe.
   *
   * @param bool $published
   *   TRUE to set this Recipe to published, FALSE to set it to unpublished.
   *
   * @return \Drupal\mentor\Entity\RecipeInterface
   *   The called Recipe entity.
   */
  public function setPublished($published);

  /**
   * Gets the Recipe revision creation timestamp.
   *
   * @return int
   *   The UNIX timestamp of when this revision was created.
   */
  public function getRevisionCreationTime();

  /**
   * Sets the Recipe revision creation timestamp.
   *
   * @param int $timestamp
   *   The UNIX timestamp of when this revision was created.
   *
   * @return \Drupal\mentor\Entity\RecipeInterface
   *   The called Recipe entity.
   */
  public function setRevisionCreationTime($timestamp);

  /**
   * Gets the Recipe revision author.
   *
   * @return \Drupal\user\UserInterface
   *   The user entity for the revision author.
   */
  public function getRevisionUser();

  /**
   * Sets the Recipe revision author.
   *
   * @param int $uid
   *   The user ID of the revision author.
   *
   * @return \Drupal\mentor\Entity\RecipeInterface
   *   The called Recipe entity.
   */
  public function setRevisionUserId($uid);

}
